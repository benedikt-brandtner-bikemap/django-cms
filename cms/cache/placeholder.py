...
def _placeholder_cache_key(placeholder, lang, unit, logged_in):
    if logged_in:
        logged_in = 'logged_in'
    else:
        logged_in = 'anonymous'

    cache_key = '%srender_placeholder:%s.%s.%s.%s' % (get_cms_setting("CACHE_PREFIX"), placeholder.pk, str(lang), str(unit), logged_in)
    if settings.USE_TZ:
...
def set_placeholder_cache(placeholder, request, lang, content):
    ...
    super_user = request.COOKIES.get(settings.LOGGED_IN_CACHE_COOKIE_NAME, False)
...
def _placeholder_page_cache_key(page_lookup, lang, unit, site_id, placeholder_name, logged_in):
    if logged_in:
        logged_in = 'logged_in'
    else:
        logged_in = 'anonymous'
    base_key = _get_cache_key('_show_placeholder_for_page', page_lookup, lang, site_id)
    base_key += '_unit:{0}_type:{1}'.format(str(unit), logged_in)
